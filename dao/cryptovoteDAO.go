...
func (dao CryptoVoteDAO) GetDbService() interfaces.InterfaceDbService {
	return dao.dbService
}

func (dao CryptoVoteDAO) SetDbService(service interfaces.InterfaceDbService) {
...
/*
	...
	using mongodbClient.
*/
func (dao CryptoVoteDAO) Create(cryptoVote model.CryptoVote) (model.CryptoVote, error) {
	var savedCryptoVote model.CryptoVote

	// uses function from dao package to start the mongo client
	mongodbClient, err := GetMongoClientInstance(dao.dbService)
	if err != nil {
	...
	// uses function from dao package to start the mongo client
	savedCryptoVote, err = dao.FindOne(filter)
	if err != nil {
		z := "[cryptovote.mongodb] Problemas na execução de dao.FindOne: " + err.Error()
		err = errors.New(z)
		return savedCryptoVote, err
	}
	return savedCryptoVote, err
...
*/
func (dao CryptoVoteDAO) FindOne(filter bson.M) (model.CryptoVote, error) {
...
/*
	...
	filter := bson. M{"symbol": "KLV"}
*/
func (dao CryptoVoteDAO) FindMany(filter bson.M) ([]model.CryptoVote, error) {
	var oneCryptoVote model.CryptoVote
...
*/
func (dao CryptoVoteDAO) FindManyLimitedSortedByField(filter bson.M, limit int64, sortFieldName string, orderType int) ([]model.CryptoVote, error) {
	var oneCryptoVote model.CryptoVote
	var manyCryptoVotes []model.CryptoVote
	var err error
...
*/
func (dao CryptoVoteDAO) UpdateOne(filter bson.M, newData bson.M) (model.CryptoVote, error) {
	var oneCryptoVote model.CryptoVote
	var err error
...
/*
	...
	filter := bson. M{"symbol": "KLV"}
*/
func (dao CryptoVoteDAO) DeleteMany(filter bson.M) (int64, error) {
