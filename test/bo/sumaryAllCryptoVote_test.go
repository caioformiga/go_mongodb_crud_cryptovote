...
func testSumaryAllCryptoVote2_SortedDesc(t *testing.T) {
	...
	prim := list[0]
	assert.Equal(t, prim.Symbol, "KLV", "they should be equal")

	seg := list[1]
	assert.Equal(t, seg.Symbol, "BTC", "they should be equal")

	ult := list[2]
	assert.Equal(t, ult.Symbol, "ETH", "they should be equal")
}
